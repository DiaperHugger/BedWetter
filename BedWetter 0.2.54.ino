/*BedWETTER v0.2.54  Updated 5/14/21
...
void EEPROM_RECALL() //EEPROM address layout tied to MenuPosition!  +10 per row.
{
  ...
  CONTROLVar[0].OutputInvert = EEPROM.read(33);
  CONTROLVar[0].DutyCycle = EEPROM.read(34);
  CONTROLVar[0].DutyPulse = EEPROM.read(35);
...
void loop()
{
    ...
    //     Serial.println(INTERVALTimes[CurrentInterval - 1]);
    //   }
    //   Serial.print(CONTROLVar[0].OutputInvert);
...
void Events() //Triggered by select button and end of EventTrigger stop time
{
    ...
    if(MakeTimes == 1)//Derive interval times
    {
      if(WETTINGVar[0].Random == 0)//None random interval times
      {
        ...
        {
            int T = ((TIMESVar[0].TotalSleepTime * 3600) / Interval);
            T = (T * i) - Duration;
      ...
      if(WETTINGVar[0].Random == 1)//If initial is present then add to first interval.
      {
        INTERVALTimes[Interval - 1] = (INTERVALTimes[Interval - 1] + (TIMESVar[0].InitialDelay * 60));
      }

      for (int i = 1; i <= Interval; i++)
      {
        Serial.print(INTERVALTimes[Interval - i]);
        Serial.print(" , ");
        Serial.println(i);
      }
...
void LCDDisplayData() //MenuPosition = ROW,COLUMN
{
    ...
    else if ((MenuPosition == 33) && (MenuPosition != LastMenuPosition)) //CONTROL OutputInvert:
    {
      lcd.setCursor(0, 1);
      lcd.print(CONTROLItems[Column]);
    ...
    else if ((MenuPosition == 34) && (MenuPosition != LastMenuPosition)) //CONTROL Duty Cycle:
    {
      lcd.setCursor(0, 1);
      lcd.print(CONTROLItems[Column]);
      lcd.setCursor(15, 1);
    ...
    else if ((MenuPosition == 35) && (MenuPosition != LastMenuPosition)) //CONTROL Duty Pulse:
    {
      lcd.setCursor(0, 1);
      lcd.print(CONTROLItems[Column]);
      lcd.setCursor(15, 1);
...
void ALTERNATE()
{
  ...
  if(MenuPosition == 33)//OutputInvert
  {
    if(LastButtonPress == 2)//UP
      {
        CurrentVal++;
