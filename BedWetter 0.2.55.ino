/*BedWETTER v0.2.55  Updated 5/16/21
...
float FloodMargin = 0.15;//15% +/-
uint8_t FloodEvents = 3;
...
String PROFILEItems[] = {"FLOOD%          ", "FLOOD DUR/s:    ", "DAMP%           ", "DAMP DUR/s:     ", "SPURTS%         ", "SPURTS DUR/s:   ", "DRY%            "};
...
void EEPROM_RECALL() //EEPROM address layout tied to MenuPosition!  +10 per row.
{
  ...
  PROFILEVar[0].Damp = EEPROM.read(32);
  PROFILEVar[0].DampDuration = EEPROM.read(33);
  PROFILEVar[0].Spurts = EEPROM.read(34);
  PROFILEVar[0].SpurtsDuration = EEPROM.read(35);
  PROFILEVar[0].Dry = EEPROM.read(36);
...
void Events() //Triggered by select button and end of EventTrigger stop time
{
    ...
    uint16_t Interval = WETTINGVar[0].WetInterval;
    ...
    if(MakeTimes == 1)//Derive interval times
    {
      ...
      {
        uint16_t ProfileIntervals =(random(0, (FloodEvents * 1024)) / 1024) + 1;//Find number of weetings
        ...
        for (int i = (ProfileIntervals - 1); i >= 0; i--)
        {
          ...
          DURATIONTimes[i] = d;
          uint16_t t = random(1, TIMESVar[0].TotalSleepTime * 3600);
          INTERVALTimes[i] = t;
        } 
...
void LCDDisplayData() //MenuPosition = ROW,COLUMN
{
    ...
    else if ((MenuPosition == 32) && (MenuPosition != LastMenuPosition)) //Profile Damp
    {
      lcd.setCursor(0, 1);
      lcd.print(PROFILEItems[Column]);
      lcd.setCursor(15, 1);
    ...
    else if ((MenuPosition == 33) && (MenuPosition != LastMenuPosition)) //Profile DampDuration
    {
      lcd.setCursor(0, 1);
      lcd.print(PROFILEItems[Column]);
      lcd.setCursor(15, 1);
    ...
    else if ((MenuPosition == 36) && (MenuPosition != LastMenuPosition)) //Profile Dry
    {
      lcd.setCursor(0, 1);
      lcd.print(PROFILEItems[Column]);
      lcd.setCursor(15, 1);
...
void CHECKLIMITS()
{
  ...
  if(MenuPosition == 30)//Flood
  {
    if(CurrentVal >= ProfileDistribution)
    {
      CurrentVal = ProfileDistribution;
    }
  }
...
{
  ProfileDistribution = (100 - (((PROFILEVar[0].Flood + PROFILEVar[0].Damp) + PROFILEVar[0].Spurts) + PROFILEVar[0].Dry));
